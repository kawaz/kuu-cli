...
- **C-1 + M-1**: 解消済み (kuu.mbt commit b3feaf0a / 70fe5670 で `front_door.parse` に 2 段後段処理 + `resolve` 追加 / `is_sentinel` + `warnings_structured` を pub 化、kuu-cli commit 5c954b58 で `wire.mbt` を 2 段呼び出しに追随)
- **M-5** (validate 余剰引数黙殺 / stdout 契約): 解消済み (commit b099a27d、`run_validate` が `args.length() > 1` で usage_die、`libc write(2)` を extern で束縛して stderr (fd=2) に human text、stdout は machine JSON only)
- **m-4** (`-h` short alias): 解消済み (commit b099a27d、削除)
- **n-1** (仮置き番号ずれ): 解消済み (commit b099a27d、`wire.mbt` 実装コメントを header と揃えて `#1` に)
- **n-2** (README PoC 数の自己矛盾): 解消済み (commit b099a27d、`impl/mbt/README.md` を「two PoC notes」に + stdout/stderr 分離 / `-h` alias 不採用の宣言追記)
- **M-3** (CI SHA-pin checkout 拒否): 解消済み (commit ae558c23、`impl/mbt/justfile setup` の判定順序を刷新 = real directory 先受理 → symlink refresh → sibling 検査 の順)
- **m-1** (e2e binary path 不一致): 解消済み (commit ae558c23、`moon build --target native` の debug 出力に合わせて exact path 固定、`find|head -1` fallback を削除)
- **m-2** (ensure-clean が push gate に未接続): 解消済み (commit ae558c23、`bump-semver vcs push` は clean 保証しない実機確認結果を踏まえて、root justfile の `push` deps に `ensure-clean` を追加)
- **M-2** (e2e が expect を審判にしていない): 解消済み (commit 5010420c、`impl/mbt/justfile e2e` を case id 経由の fixture 審判化に書き換え = args / expect を fixture 本体から `jq` で読み込む形に。pin 5 case = separator-typed + rename + collision (C-1 の再発検知軸) + inheritable basic (C-1 の再発検知軸) + deprecated (M-1 の再発検知軸))
- **m-3** (README fixture 接続範囲の過大表現): 解消済み (commit 5010420c、`impl/mbt/README.md` の e2e 節を「5 fixture の代表 case を case id 経由で消費」の実態記述に更新、root `README.md` / `README-ja.md` の impl 表も同期)
- **M-4** + 「v1 契約正本化前に決めること」8 項目: 未着手 (今サイクルの射程外、kuu-ux 設計と合わせて別サイクル、統括裁定 2026-07-16 で再確認)
